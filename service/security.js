...
var socialLogin = function(provider, req, res, next){
  ...
  workflow.on('logUserIn', function(){
    ...
    req.login(workflow.user, function(err) {
      ...
      workflow.outcome.defaultReturnUrl = app.models.User.defaultReturnUrl(); // FIX THIS: workflow.user.defaultReturnUrl();
...
/*
    ...
    workflow.on('duplicateEmailCheck', function() {
      req.app.db.users.findOne({ email: req.body.email.toLowerCase() }, function(err, user) {
        if (err) {
          return workflow.emit('exception', err);
        }
    ...
    workflow.on('createUser', function() {
      req.app.models.User.encryptPassword(req.body.password, function(err, hash) {
        if (err) {
          return workflow.emit('exception', err);
        }
        var fieldsToSet = {
        ...
        req.app.db.users.insert(fieldsToSet, function(err, user) {
          if (err) {
            return workflow.emit('exception', err);
          }
    ...
    workflow.on('createAccount', function() {
      ...
      req.app.db.accounts.insert(fieldsToSet, function(err, account) {
        if (err) {
          return workflow.emit('exception', err);
        }

        //update user with account
        req.app.db.users.save({id: workflow.user.id, role_account: account.id}, function(err, user) {
          if (err) {
            return workflow.emit('exception', err);
          }
      ...
      req._passport.instance.authenticate('local', function(err, user, info) {
        ...
        if (!user) {
          ...
          req.login(user, function(err) {
            ...
            workflow.outcome.defaultReturnUrl = app.models.User.defaultReturnUlr(); // FIX THIS user.defaultReturnUrl();
            workflow.emit('response');
          });
        }
      })(req, res);
  ...
  /* PENDING: Fix admin roles; in the passport deserialize function */
  ...
  login: function(req, res){
    ...
    workflow.on('abuseFilter', function() {
      var getIpCount = function(done) {
        ...
        req.app.db.login_attempts.count(conditions, function(err, count) {
          if (err) {
            return done(err);
          }
      ...
      var getIpUserCount = function(done) {
        var conditions = { ip: req.ip, username: req.body.username };
        req.app.db.login_attempts.count(conditions, function(err, count) {
          if (err) {
            return done(err);
          }
    ...
    workflow.on('attemptLogin', function() {
      req._passport.instance.authenticate('local', function(err, user, info) {
        ...
        if (!user) {
          var fieldsToSet = { ip: req.ip, username: req.body.username };
          req.app.db.login_attempts.insert(fieldsToSet, function(err, doc) {
            if (err) {
              return workflow.emit('exception', err);
            }
          ...
          req.login(user, function(err) {
            ...
            workflow.outcome.user = filterUser(req.user);
            workflow.outcome.defaultReturnUrl = req.app.models.User.defaultReturnUrl(); // user.defaultReturnUrl(); FIX THIS
            workflow.emit('response');
          });
        }
      })(req, res);
